#include "navmanger.h"

const unsigned char auchCRCHi[] = {
    };
const unsigned char auchCRCLo[] = {
    };
unsigned short CRC16(char *puchMsg, unsigned short usDataLen)
{
    unsigned char uchCRCHi = 0xFF ;
    unsigned char uchCRCLo = 0xFF ;
    unsigned uIndex;
    while (usDataLen--)
    {
        uIndex = uchCRCHi ^ *puchMsg++ ;
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex];
        uchCRCLo = auchCRCLo[uIndex];
    }
    return (uchCRCHi << 8 | uchCRCLo) ;
}

void NavManger::SetCanTurn(bool direct)
{
    for(int i=0;i<m_nvnuber;i++)
    {
        getnav[i].SetCanTurn(direct);
    }
}

NavManger::NavManger(int nvmuber, QObject *parent) :
    QObject(parent)
{
    m_nvnuber=nvmuber;
    QString comname="/dev/ttySP4";
    m_serial =new xsserial(this);
    nvcal=new NvCal[nvmuber];
    getnav=new GetNavigation[nvmuber];
    m_serial->Open(comname,9600);
    connect(&m_threadTime ,SIGNAL(TimeoutEvent()),this,SLOT(GetReadValue()));
    m_threadTime.startrun(50);
}

void NavManger::GetSendValue(unsigned char *sendvalue,int no)
{
    sendvalue[0]=0x02;
    sendvalue[1]=0x03;
    sendvalue[2]=0x00;
    sendvalue[3]=0x02;
    sendvalue[4]=0x00;
    sendvalue[5]=0x02;
    sendvalue[6]=0x65;
    sendvalue[7]=0xF8;
}
float NavManger::GetNavigationValue(int devno)
{
    float nv=-100000;
    if(devno<m_nvnuber)
    {
         QTime ReadTime = QTime::currentTime();
        int elapsed = ReadTime.msecsTo(nvcal[devno].caltime);
        elapsed=qAbs(elapsed);
        if(elapsed<1000)
        {
           nv= getnav[devno].GetNavigationValue();
        }
    }
    return nv;
}

int runnubmer=0;
void NavManger::GetReadValue()
{

    runnubmer++;

    int devno=runnubmer%m_nvnuber;

    unsigned char WillSendValue[8];
    GetSendValue(WillSendValue,devno);

    m_serial->serial_write((char*)WillSendValue,8);
    m_threadTime.mmsleep(40);
    int readlen=m_serial->serial_read();

    QVector<uchar> qu;
    for(int i=0;i<readlen;i++)
    {
        qu.append(m_serial->uart_buff[i]);
    }

    if(readlen==8)
    {
        char tempchar[8];
        for(int i=0;i<8;i++)
        {
            tempchar[i]=m_serial->uart_buff[i];
        }
        m_serial->uart_buff[4]=0;
        for(int i=5;i<9;i++)
        {
            m_serial->uart_buff[i]=tempchar[i-1];
        }

        char sendchar[7];

        for(int i=0;i<7;i++)
        {
            sendchar[i]=m_serial->uart_buff[i];

        }
        unsigned short crc= CRC16(sendchar,7);
        unsigned char cch=crc>>8;
        unsigned char ccl=crc;

        unsigned char uch=(unsigned char)m_serial->uart_buff[7];
        unsigned char ucl=(unsigned char)m_serial->uart_buff[8];
        if(cch==uch&&ccl==ucl)
        {
            unsigned char b1=m_serial->uart_buff[5];
            unsigned char b2=m_serial->uart_buff[6];
            b1=~b1;
            b2=~b2;
            getnav[devno].Transition(b2,b1);
            nvcal[devno].caltime=QTime::currentTime();
        }

    }
}
