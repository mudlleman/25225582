...
void GetNavigation::GetReadValue()
{
    ...
    unsigned char WillSendValue[5];
    WillSendValue[0]=0x02;
    WillSendValue[1]=0x03;
    WillSendValue[2]=0x00;
    WillSendValue[3]=0x02;
    WillSendValue[4]=0x70;
    WillSendValue[5]=0x5d;


    m_serial->serial_write((char*)WillSendValue,6);
    ...
    if(readlen==6)
    {
        ...
        unsigned short crc= CRC16(sendchar,4);
        unsigned char cch=crc>>8;
        unsigned char ccl=crc;

        unsigned char uch=(unsigned char)m_serial->uart_buff[4];
        unsigned char ucl=(unsigned char)m_serial->uart_buff[5];

        if(cch==uch&&ccl==ucl)
        {

            unsigned char b1=m_serial->uart_buff[2];
            unsigned char b2=m_serial->uart_buff[3];
            b1=~b1;
            b2=~b2;
            Transition(b2,b1);
        }
...
float GetNavigation::GetNvflaotvalue(QList<nvstrcut> curlist,bool directsmall)
{
    ...
    else
    {
        ...
        int starttemp=strcutnv.endno-points;
...
{
    float turenumber=0;
    for(int i=srart;i<srart+len+1;i++)
    {
        turenumber+=i;
    }
