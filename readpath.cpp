...
void ReadPath::GetPath(QDomElement &docElem)
{
    ...
    while(!node.isNull())
    {
        ...
        if( element.tagName()=="pointdirect" )
        {
            ...
            cd.leftlen=0;
            if( element.hasAttribute("leftlen") )
                cd.leftlen= element.attributeNode("leftlen").value().toInt();
             cd.rightlen= 0;
            if( element.hasAttribute("rightlen") )
                cd.rightlen= element.attributeNode("rightlen").value().toInt();
...
PointRuntype ReadPath::getretruntype(bool direct, int pathid, int toid,int steplen)
{
    PointRuntype prtype;
    prtype.isright=direct;
    prtype.pathid=pathid;
    prtype.topathid=toid;
    prtype.steplen=steplen;
    return  prtype;
}
...
void ReadPath::getpath(int preid, PointPathStruct ppst, int tagid, QList<PointRuntype> lsrunright, QList<PointRuntype> lsrunleft)
{
    ...
    if( it != m_pathmap.end())
    {
        ...
        PointRuntype prtype=getretruntype(false,ppst.pathid,pp.leftid,pp.leftlen);
    ...
    {
        PointPathStruct p = it.value();
        PointRuntype prtype=getretruntype(true, ppst.pathid, pp.rightid,pp.rightlen);
